...
# Default paths
DEFAULT_VISUAL_PATH = './dataset/train_h5/tvsum_video_clip.h5'
DEFAULT_TEXT_PATH = './dataset/train_h5/tvsum_text_clip.npy'
DEFAULT_SPLIT_PATH = './dataset/train_h5/splits/'


class VideoData(Dataset):
    def __init__(self, name, mode, split_index, visual_path=DEFAULT_VISUAL_PATH, text_path=DEFAULT_TEXT_PATH):
        self.mode = mode
        self.name = name
        ...
        # Load visual features once (if small enough)
        self.visual_data = {}
...
def collate_fn(batch):
    ...
    for item in batch:
        ...
        if len(item) > 2:  # If name is included
            video_names.append(item[2])
        else:
            video_names.append(f"video_{len(video_names)}")  # Default name
    
    # Padding sequences
    padded_videos = pad_sequence(video_seqs, batch_first=True)  # [B, T_v_max, 1024]
    padded_texts = pad_sequence(text_seqs, batch_first=True)    # [B, T_t_max, 768]
...
def get_loader(name, mode, split_index, batch_size=1, shuffle=False,
               visual_path=DEFAULT_VISUAL_PATH, text_path=DEFAULT_TEXT_PATH):
    
    dataset = VideoData(name, mode, split_index, visual_path, text_path)
...
def get_loader_custom_video_data(mode, visual_path, text_path, batch_size=1, shuffle=False):
