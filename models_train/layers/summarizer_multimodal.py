...
# Scaled dot-product attention scores (from Code1)
def scaled_dot_product_attention(q, k, v, dk, attention_mask=None, dropout=0.2, training=True):
    ...
    #  dropout to attention probabilities (for regularization)
    scores = F.dropout(scores, p=dropout, training=training)
    
    # Return the final weighted output
...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, attention_size, num_heads=8, input_size=None, dropout=0.1):
        ...
        # Projection to combine attention heads after processing
        self.head_projection = nn.Linear(attention_size, attention_size)
        
        # Output layer to compute the final scalar (importance score)
    ...
    def forward(self, x, attention_mask=None):
        ...
        if attention_mask is not None and len(attention_mask.size()) == 2:
            attention_mask = attention_mask.unsqueeze(1).unsqueeze(1)  # Ensure mask is correctly shaped
        ...
        # Compute final scalar score for each frame (to represent importance)
...
class Summarizer(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=2):
        ...
        self.fusion_module = CrossModalFusion(video_dim=512, text_dim=512, hidden_size=hidden_size, num_heads=8)
    ...
    def forward(self, video_feats, text_feats, mask_fused=None, uniform=False):
        ...
        fused_video, fused_text = self.fusion_module(video_feats, text_feats, mask_fused)

        
        # use text-guided video
        fused = fused_video.permute(1, 0, 2)
