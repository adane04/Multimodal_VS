...
class CrossModalFusion(nn.Module):
    def __init__(self, video_dim, text_dim, hidden_size=512, num_heads=8, dropout=0.1, n_layers=3):
        ...
        self.attn_fusion = nn.ModuleList([
            ...
            for _ in range(n_layers) ## multi-layer cross-modal attention
        ])
    ...
    def forward(self, video_feats, text_feats, mask_text=None):
        ...
        Returns:
            ...
            text_proj: [B, N_text, hidden_size]
        """
        ...
        # Language-guided attention with stacking
        for i in range(self.n_layers):
            attended_video = self.attn_fusion[i](v_proj, t_proj, t_proj, mask=mask_text)
            v_proj = self.norm_layers[i](v_proj + attended_video)
